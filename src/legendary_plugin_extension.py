from gimpfu import (pdb, gimp, RGBA_IMAGE, NORMAL_MODE, GRAYA_IMAGE, register, main, RGB, CHANNEL_OP_SUBTRACT,
                    BUCKET_FILL_BG, LAYER_MODE_NORMAL, PF_IMAGE, PF_COLOR, STROKE_LINE, PF_INT, PF_BOOL, PF_FONT,
...
def draw_label(image, group, font, color, label_text, font_size, top_left_x, top_left_y, width, height):
    cell_label_layer = pdb.gimp_text_layer_new(image, label_text, font, font_size, 0)
...
def draw_single_cell_with_label(image, font, label_text, top_left_x, top_left_y, color):
    group = create_group_layer(image, label_text)
    if len(label_text) > 0:
        draw_label(image, group, font, color, label_text, 54, top_left_x, top_left_y, CELL_WIDTH, LABEL_HEIGHT)
    draw_cell(image, group, label_text, top_left_x, top_left_y)


def draw_large_cell(image, font, color, text, top_left_x, top_left_y):
    ...
    draw_label(image, group, font, color, text, 150, top_left_x, top_left_y, LARGE_CELL_WIDTH, 200)
    return large_layer, group


def draw_city(image, font, color, city_x, city_y):
    city_layer, group = draw_large_cell(image, font, color, 'City', city_x, city_y)
    ...
    for city_label in city_labels:
        draw_label(image, group, font, color, city_label, 54, current_x_pos, label_y_position + label_box_height / 3,
...
def draw_single_cells(image, single_cells, font, color, progress):
    for single_cell in single_cells:
        draw_single_cell_with_label(image, font, single_cell[0], single_cell[1], single_cell[2], color)
        progress += 0.05
        gimp.progress_update(progress)


def draw_hq_and_city(image, font, color, large_cell_x):
    hq_y = image.height - OUTSIDE_GAP - LARGE_CELL_HEIGHT
    draw_large_cell(image, font, color, 'HQ', large_cell_x, hq_y)
    gimp.progress_update(0.75)
    city_y = hq_y - ROW_GAP - LARGE_CELL_HEIGHT
    draw_city(image, font, color, large_cell_x, city_y)
    gimp.progress_update(1)


def draw_legendary_playmat_28_by_14(image, filename, opacity, font, color, include_extra_cell):
    ...
    if include_extra_cell is False:
        single_cells.pop(0)
    draw_single_cells(image, single_cells, font, color, progress)
    large_cell_x = OUTSIDE_GAP + CELL_WIDTH * 2 + COLUMN_GAP + MAIN_SECTION_GAP
    draw_hq_and_city(image, font, color, large_cell_x)


def draw_legendary_playmat_24_by_14(image, filename, opacity, font, color):
    LAST_COLUMN_X = image.width - OUTSIDE_GAP - CELL_WIDTH
    SECOND_LAST_COLUMN_X = LAST_COLUMN_X - CELL_WIDTH - COLUMN_GAP
...
def redraw_single_cell_layer_group(image, new_cell_label_text, font, color):
    ...
    draw_single_cell_with_label(image, font, new_cell_label_text, x, y, color)
...
register(
  ...
  [                                                         # Parameters
      ...
      (PF_FONT, "font", "Label Font", 'Percolator Medium'),
